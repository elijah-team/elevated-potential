...
public class DefaultParser implements CommandLineParser {
	...
	/**
	 * A nested builder class to create {@code DefaultParser} instances using
	 * descriptive methods.
	 * <p>
	 * Example usage:
	 * 
	...
	public static final class Builder {
		...
		/**
		 * Flag indicating if balanced leading and trailing double quotes should be
		 ...
		 * <p>
		 * Both allowPartialMatching and stripLeadingAndTrailingQuotes are true by
		 ...
		 * {@code -de} would enable both {@code debug} as well as {@code extract}
		 *
		...
		/**
		 * Sets if balanced leading and trailing double quotes should be stripped from
		 * option arguments.
		 * <p>
		 * If "stripping of balanced leading and trailing double quotes from option
		 * arguments" is true, the outermost balanced double quotes of option arguments
		 * values will be removed. For example, {@code -o '"x"'} getValue() will return
		 * {@code x}, instead of {@code "x"}
		 * <p>
		 * If "stripping of balanced leading and trailing double quotes from option
		 * arguments" is null, then quotes will be stripped from option values separated
		 * by space from the option, but kept in other cases, which is the historic
		 * behavior.
		 *
		 * @param stripLeadingAndTrailingQuotes whether balanced leading and trailing
		 *                                      double quotes should be stripped from
		 *                                      option arguments.
		 * @return this builder, to allow method chaining
		 * @since 1.5.0
		 */
	...
	/**
	 * Flag indicating if balanced leading and trailing double quotes should be
	 * stripped from option arguments. null represents the historic arbitrary
	 * behavior
	 */
	...
	/**
	 * Flag indicating how unrecognized tokens are handled. {@code true} to stop the
	 * parsing and add the remaining tokens to the args list. {@code false} to throw
	 * an exception.
	 */
	...
	/**
	 * The required options and groups expected to be found when parsing the command
	 * line.
	 */
	 ...
	 * options.
	 *
	 * @param allowPartialMatching if partial matching of long options shall be
	 *                             enabled
	 */
	...
	/**
	 * Creates a new DefaultParser instance with the specified partial matching and
	 * quote stripping policy.
	 *
	 * @param allowPartialMatching          if partial matching of long options
	 *                                      shall be enabled
	 * @param stripLeadingAndTrailingQuotes if balanced outer double quoutes should
	 *                                      be stripped
	 */
	...
	/**
	 * Gets a list of matching option strings for the given token, depending on the
	 * selected partial matching policy.
	 *
	 * @param token the token (may contain leading dashes)
	 * @return the list of matching option strings or an empty list if no matching
	 ...
	 * <li>for each remaining character check if an {@link Option} exists with that
	 * id.</li>
	 * <li>if an {@link Option} does exist then add that character prepended with
	 * "<b>-</b>" to the list of processed tokens.</li>
	 * <li>if the {@link Option} can have an argument value and there are remaining
	 * characters in the token then add the remaining characters as a token to the
	 * list of processed tokens.</li>
	 * <li>if an {@link Option} does <b>NOT</b> exist <b>AND</b>
	 * {@code stopAtNonOption} <b>IS</b> set then add the special token "<b>--</b>"
	 * followed by the remaining characters and also the remaining tokens directly
	 * to the processed tokens list.</li>
	 * <li>if an {@link Option} does <b>NOT</b> exist <b>AND</b>
	 * {@code stopAtNonOption} <b>IS NOT</b> set then add that character prepended
	 * with "<b>-</b>".</li>
	 * </ul>
	 *
		...
		if (t.length() == 1) {
			...
			if (options.hasShortOption(t)) {
				...
				if (opt != null && options.getOption(opt).acceptsArg()) {
					...
					currentOption
							.addValueForProcessing(stripLeadingAndTrailingQuotesDefaultOff(t.substring(opt.length())));
					currentOption = null;
	...
	private void handleToken(final @NotNull CompilerInput ci) throws ParseException {
		...
		if (skipParsing) {
			...
			skipParsing = true;
		} else if (currentOption != null && currentOption.acceptsArg() && isArgument(argument)) {
			currentOption.addValueForProcessing(stripLeadingAndTrailingQuotesDefaultOn(argument));
		} else if (argument.startsWith("--")) {
			handleLongOption(argument);
		} else if (argument.startsWith("-") && !"-".equals(argument)) {
			ci.setArg();
			handleShortAndLongOption(argument);
		} else {
			handleUnknownToken(ci);
		}
	 ...
	 * arguments of the command line. If the stopAtNonOption flag is set, this stops
	 * the parsing and the remaining tokens are added as-is in the arguments of the
	 * command line.
	 *
	 * @param token the command line token to handle
	 */
	 ...
	 * @param token
	 */
	private boolean isLongOption(final @Nullable String token) {
	 ...
	 * @param token
	 */
	private boolean isShortOption(final @Nullable String token) {
	...
	public CommandLine parse(@NotNull Options options, @NotNull List<CompilerInput> aInputs) throws ParseException {
		...
		final List<String> arguments = aInputs.stream().map(inp -> inp.getInp()).collect(Collectors.toList());

		if (arguments != null) {
			for (final CompilerInput argument : aInputs) {
	 ...
	 *                        parsing and the remaining arguments are added to the
	 *                        {@link CommandLine}s args list. If {@code false} an
	 ...
	 * stripLeadingAndTrailingQuotes is set If stripLeadingAndTrailingQuotes is
	 * null, then do not strip
	 *
	...
	private void updateRequiredOptions(final @NotNull Option option) throws AlreadySelectedException {
		...
		// if the option is in an OptionGroup make that option the selected option of
		// the group
